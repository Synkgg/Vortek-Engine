...
void DirectoryWatcher::Impl::RunWindows()
{
	...
	{
		VORTEK_ERROR( "Failed to open directory: {}", GetLastError() );
		return;
	}
	...
	while ( !bStopFlag )
	{
		...
		if ( !hEvent )
		{
			VORTEK_ERROR( "Failed to receive directory notifications." );
			break;
		}
		...
		{
			VORTEK_ERROR( "ReadDirectoryChangesW failed: {}", GetLastError() );
			break;
		}
		...
		if ( waitStatus == WAIT_OBJECT_0 ) // Overlapped event was signalled
		{
			...
			if ( !GetOverlappedResult( hDir, &overlapped, &bytesReturned, FALSE ) )
			{
				...
				VORTEK_ERROR( "GetOverlappedResult failed: {}", err );
				break;
			}
		...
		{
			VORTEK_ERROR( "WaitForMultipleObjects failed: {}", GetLastError() );
			break;
		}
	}
