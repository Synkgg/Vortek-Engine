...
namespace VORTEK_FILESYSTEM
{
		...
		, m_bNewLineAdded{ false }
	{
		...
		if (!m_FileStream.is_open())
			throw std::runtime_error(std::format("LuaSerialization failed. Failed to open file [{}]", sFilepath));
	}
	...
	{
		VORTEK_ASSERT(m_NumTablesStarted == 0 && "Too many tables started! Did you forget to call EndTable()?");
		VORTEK_ASSERT(m_NumIndents == 0 && "Indent count should be zero when ending the document!");
		Stream(NEW_LINE);
		return true;
	}
