...
void StateMachine::ChangeState( const std::string& stateName, bool bRemoveState, const sol::object& enterParams )
{
	...
	{
		VORTEK_ERROR( "Failed to change states. [{}] does not exist.", stateName );
		return;
	}
	...
	else
	{
		...
		if ( oldState->on_exit.valid() )
		{
			...
			{
				VORTEK_ERROR( "Failed to exit state: {}", error.what() );
				return;
			}
		}
	...
	if ( newState->on_enter.valid() )
	{
		...
		{
			VORTEK_ERROR( "Failed to enter state: {}", error.what() );
			return;
		}
	}
}
...
void StateMachine::Update( const float dt )
{
	...
	{
		VORTEK_ERROR( "Failed to update state: {}", error.what() );
	}
	catch ( ... )
	{
		VORTEK_ERROR( "Failed to update state: Unknown error." );
	}
}
...
void StateMachine::Render()
{
	...
	{
		VORTEK_ERROR( "Failed to render state: {}", error.what() );
	}
	catch ( ... )
	{
		VORTEK_ERROR( "Failed to render state: Unknown error." );
	}
}
...
void StateMachine::AddState( const State& state )
{
	...
	{
		VORTEK_ERROR( "Failed to add state: {} -- Already exists.", state.sName );
		return;
	}
...
void StateMachine::ExitState()
{
	...
	if ( stateItr == m_mapStates.end() )
	{
		VORTEK_ERROR( "Failed to exit state: {} -- State does not exist.", m_sCurrentState );
		return;
	}
	auto& stateRef = stateItr->second;
