...
bool ScriptingSystem::LoadMainScript( const std::string& sMainLuaFile, Vortek::Core::ECS::Registry& registry,
									  sol::state& lua )
{
	std::error_code ec;
...
bool ScriptingSystem::LoadMainScript( Vortek::Core::ProjectInfo& projectInfo, Vortek::Core::ECS::Registry& registry,
									  sol::state& lua )
{
	...
	auto optContentPath = projectInfo.TryGetFolderPath( Vortek::Core::EProjectFolderType::Content );
...
void ScriptingSystem::Update( Vortek::Core::ECS::Registry& registry )
{
	if ( !m_bMainLoaded )
	{
...
void ScriptingSystem::Render( Vortek::Core::ECS::Registry& registry )
{
	if ( !m_bMainLoaded )
	{
		VORTEK_ERROR( "Main lua script has not been loaded!" );
...
auto create_lua_logger = []( sol::state& lua ) {
	auto& logger = Vortek::Logger::Logger::GetInstance();

	lua.new_usertype<Vortek::Logger::Logger>(
		"Logger",
		sol::no_constructor,
		"log",
    ...
    assert = function(arg1, message, ...)
        ...
        return original_assert(arg1, message, ...)
    end
)" );
};

void ScriptingSystem::RegisterLuaBindings( sol::state& lua, Vortek::Core::ECS::Registry& registry )
{
	Vortek::Core::Scripting::GLMBindings::CreateGLMBindings( lua );
	Vortek::Core::InputManager::CreateLuaInputBindings( lua, registry );
	VORTEK_RESOURCES::AssetManager::CreateLuaAssetManager( lua );
	Vortek::Core::Scripting::SoundBinder::CreateSoundBind( lua );
	Vortek::Core::Scripting::RendererBinder::CreateRenderingBind( lua, registry );
	Vortek::Core::Scripting::UserDataBinder::CreateLuaUserData( lua );
	Vortek::Core::Scripting::LuaFilesystem::CreateLuaFileSystemBind( lua );
	Vortek::Core::Scripting::ScriptingHelpers::CreateLuaHelpers( lua );

	Vortek::Core::FollowCamera::CreateLuaFollowCamera( lua, registry );
	Vortek::Core::Character::CreateCharacterLuaBind( lua, registry );

	create_timer( lua );
	create_lua_logger( lua );
	createTweenLuaBind( lua );

	Vortek::Core::State::CreateLuaStateBind( lua );
	Vortek::Core::StateStack::CreateLuaStateStackBind( lua );
	Vortek::Core::StateMachine::CreateLuaStateMachine( lua );
	...
	{
		Vortek::Core::Scripting::ContactListenerBinder::CreateLuaContactListener( lua, registry.GetRegistry() );
		PhysicsComponent::CreatePhysicsLuaBind( lua, registry.GetRegistry() );
	}
}

void ScriptingSystem::RegisterLuaFunctions( sol::state& lua, Vortek::Core::ECS::Registry& registry )
{
	...
	lua.set_function( "MeasureText",
		sol::overload(
			...
			{
				return Vortek::Core::GetTextBlockSize(text, transform, assetManager);
			}
		)
	);
	...
	lua.set_function( "GetProjecPath", [ & ] { return engine.GetProjectPath(); } );

	lua.new_usertype<Vortek::Utilities::RandomIntGenerator>(
		"RandomInt",
		sol::call_constructor,
		sol::constructors<Vortek::Utilities::RandomIntGenerator( uint32_t, uint32_t ),
						  Vortek::Utilities::RandomIntGenerator()>(),
		"get_value",
		&Vortek::Utilities::RandomIntGenerator::GetValue );

	lua.new_usertype<Vortek::Utilities::RandomFloatGenerator>(
		"RandomFloat",
		sol::call_constructor,
		sol::constructors<Vortek::Utilities::RandomFloatGenerator( float, float ),
						  Vortek::Utilities::RandomFloatGenerator()>(),
		"get_value",
		&Vortek::Utilities::RandomFloatGenerator::GetValue );
	...
	lua.set_function( "EntityInView", [ & ]( const TransformComponent& transform, float width, float height ) {
		...
		return Vortek::Core::EntityInView( transform, width, height, *camera );
	} );

	Scene::CreateLuaBind( lua );
}

void ScriptingSystem::RegisterLuaEvents( sol::state& lua, Vortek::Core::ECS::Registry& registry )
{
...
void ScriptingSystem::RegisterLuaSystems( sol::state& lua, Vortek::Core::ECS::Registry& registry )
{
	Vortek::Core::Systems::RenderSystem::CreateRenderSystemLuaBind( lua, registry );
	Vortek::Core::Systems::RenderUISystem::CreateRenderUISystemLuaBind( lua );
	Vortek::Core::Systems::AnimationSystem::CreateAnimationSystemLuaBind( lua, registry );
}
