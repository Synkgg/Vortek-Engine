...
namespace
{
const std::string PRIVATE_KEY = "Vortek2DPrivateEncryptionKey_1234321";
...
void ScriptingHelpers::CreateLuaHelpers( sol::state& lua )
{
	...
	std::string sLuaInsertUnique = R"(
		function InsertUnique(tbl, value)
	...
	std::string sLuaMakeReadOnlyTable = R"(
		function MakeReadOnlyTable(name, entries)
	...
	try
	{
		...
		lua.set_function("Encrypt", &SimpleEncrypt);
		lua.set_function("Decrypt", &SimpleDecrypt);
	}
	catch (const sol::error& err)
	{
		VORTEK_ERROR("Failed to create lua shallow copy code. {}", err.what());
	}
