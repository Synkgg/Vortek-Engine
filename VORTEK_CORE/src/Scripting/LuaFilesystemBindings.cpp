...
void LuaFilesystem::CreateLuaFileSystemBind( sol::state& lua )
{
	lua.new_usertype<LuaFilesystem>(
		...
		[]( const std::string& sFilepath ) {
			...
			{
				VORTEK_ERROR( "Failed to remove file [{}] - Error: {}", sFilepath, ec.message() );
				return false;
			}
		...
		[]( const std::string& sDirectorPath ) {
			...
			{
				VORTEK_ERROR( "Directory [{}] already exists. - {}", sDirectorPath, ec.message() );
				return false;
			}

			if ( !fs::create_directory( sDirectorPath, ec ) )
			{
				VORTEK_ERROR( "Failed to create directory [{}]. - Error: {}", sDirectorPath, ec.message() );
				return false;
			}
		...
		[]( const std::string& sFilename, const std::string& data ) {
			...
			if ( !outFile.is_open() )
			{
				VORTEK_ERROR( "Could Not Open File: [{}]. Unable to write data to file!", sFilename );
				return false;
			}
