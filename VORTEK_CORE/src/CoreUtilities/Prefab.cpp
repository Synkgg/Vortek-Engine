...
Prefab::Prefab( EPrefabType eType, const PrefabbedEntity& prefabbed )
	...
	, m_sPrefabPath{}
{
	VORTEK_ASSERT( prefabbed.id && "Must have an ID Component" );
	VORTEK_ASSERT( !prefabbed.id->name.empty() && "All prefabs must have unique names!" );
...
bool Prefab::Load( const std::string& sPrefabPath )
{
	...
	{
		VORTEK_ERROR( "Failed to open tilemap file [{}]", sPrefabPath );
		return false;
	}
	...
	{
		VORTEK_ERROR( "Failed to load prefab file [{}] - Empty prefab files are invalid.", sPrefabPath );
		return false;
	}
	...
	{
		VORTEK_ERROR( "Failed to load prefab: File: [{}] - Prefabs must have components.", sPrefabPath );
		return false;
	}
...
bool Prefab::Save()
{
	...
	{
		VORTEK_ERROR( "Failed to save game objects[{}] - [{}]", m_sPrefabPath, ex.what() );
		return false;
	}
	...
	{
		VORTEK_ERROR( "Failed to save game objects - Filepath does not exist [{}]", m_sPrefabPath );
		return false;
	}
...
std::shared_ptr<Prefab> PrefabCreator::CreatePrefab( EPrefabType eType, VORTEK_CORE::ECS::Entity& entityToPrefab )
{
	...
	if ( auto* sprite = entityToPrefab.TryGetComponent<SpriteComponent>() )
	{
		...
		VORTEK_ASSERT( pTexture && "Sprite texture must exist in the asset manager." );
	...
	{
		VORTEK_ERROR( "Failed to create prefab. {}", ex.what() );
	}
...
bool PrefabCreator::DeletePrefab( Prefab& prefabToDelete )
{
	...
	{
		VORTEK_ERROR( "Failed to delete prefab. {}", ec.message() );
		return false;
	}

	if ( fs::is_directory( prefabPath ) || !prefabPath.has_extension() )
	{
		VORTEK_ERROR( "Failed to delete prefab. The path is a directory. To delete, we must pass in the file." );
		return false;
	}
	...
	if ( fs::exists( prefabPath ) )
	{
		VORTEK_ERROR( "Failed to delete prefab. File not deleted correctly." );
		return false;
	}
