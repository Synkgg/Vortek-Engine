...
std::tuple<int, int> ConvertWorldPosToIsoCoords( const glm::vec2& position, const Canvas& canvas )
{
	...
	*	px = x * cs - y * sn;
		py = x * sn + y * cs;
	*/
	float px = xPos * cos( VORTEK_MATH::PIOver4 ) - yPos * sin( VORTEK_MATH::PIOver4 );
	float py = xPos * sin( VORTEK_MATH::PIOver4 ) + yPos * cos( VORTEK_MATH::PIOver4 );
										   ...
										   VORTEK_RESOURCES::AssetManager& assetManager )
{
	...
	if ( !pFont )
	{
		VORTEK_ERROR( "Failed to get font [{}] - Does not exist in asset manager!", textComp.sFontName );
	...
	for ( int i = 0; i < textComp.sTextStr.size(); i++ )
	{
		...
		{
			VORTEK_ERROR( "Failed to get Text Box size correctly. Please check you text wrap, padding, textStr, etc." );
			return std::make_tuple( -1.f, -1.f );
		}
		...
		if ( text_size > 0 && ( temp_pos.x > ( wrap + pos.x ) || character == '\0' || bNewLine ) )
		{
			if ( !bNewLine )
			{
						...
						textComp.sTextStr[ i ] != '!' && textComp.sTextStr[ i ] != '?' && text_size > 0 )
				{
					...
					if ( i < 0 )
					{
						VORTEK_ERROR( "Text wrap is too small!" );
						return std::make_tuple( -1.f, -1.f );
					}
				}
			}
