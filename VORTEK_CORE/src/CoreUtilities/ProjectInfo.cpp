...
{
	VORTEK_ASSERT( fs::exists( path ) && "Project Path Must Exist!" );
	m_ProjectPath = path;
}

const fs::path& ProjectInfo::GetProjectPath() const
{
	VORTEK_ASSERT( fs::exists( m_ProjectPath ) && "Project Path was not set correctly!" );
	return m_ProjectPath;
}

void ProjectInfo::SetProjectFilePath( const fs::path& path )
{
	VORTEK_ASSERT( fs::exists( path ) && "Project File Path Must Exist!" );
...
{
	VORTEK_ASSERT( fs::exists( path ) && "Main Lua Script File Path Must Exist!" );
...
{
	VORTEK_ASSERT( fs::exists( path ) && "Script List File Path Must Exist!" );
...
{
	VORTEK_ASSERT( fs::exists( path ) && "File Icon Path Must Exist!" );
	m_FileIconPath = path;
