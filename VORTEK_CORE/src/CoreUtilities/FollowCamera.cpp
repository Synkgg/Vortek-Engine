...
FollowCamera::FollowCamera( VORTEK_RENDERING::Camera2D& camera, const ECS::Entity& entity,
	...
	, m_Entity{ entity }
{
	VORTEK_ASSERT( m_Params.scale > 0.f && "The scale must be greater than zero!" );
	...
	lua.new_usertype<FollowCamParams>(
		...
		sol::factories(
			[]( const sol::table& params ) {
				return FollowCamParams{ .minX = params[ "min_x" ].get_or( 0.f ),
										.minY = params[ "min_y" ].get_or( 0.f ),
										.scale = params[ "scale" ].get_or( 1.f ),
										.springback = params[ "springback" ].get_or( 1.f ),
										.maxX = params[ "max_x" ].get_or(
											static_cast<float>( CoreEngineData::GetInstance().WindowWidth() ) ),
										.maxY = params[ "max_y" ].get_or(
											static_cast<float>( CoreEngineData::GetInstance().WindowHeight() ) ) };
			},
