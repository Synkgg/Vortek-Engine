...
bool TilemapLoader::LoadTilemapLua( VORTEK_CORE::ECS::Registry& registry, const std::string& sTilemapFile )
{
	...
	{
		VORTEK_ERROR( "Failed to load tilemap map file: [{}], {}", sTilemapFile, err.what() );
		return false;
	}
	...
	if ( !maybeTiles )
	{
		VORTEK_ERROR( "Failed to load tilemap map file: \"tilemap\" table is missing or invalid." );
		return false;
	}
	...
	for ( const auto& [ key, value ] : *maybeTiles )
	{
		...
		if ( !components )
		{
			VORTEK_ERROR( "Failed to load object map file: \"components\" table is missing or invalid." );
			return false;
		}
...
bool TilemapLoader::LoadObjectMapLua( VORTEK_CORE::ECS::Registry& registry, const std::string& sObjectMapFile )
{
	...
	{
		VORTEK_ERROR( "Failed to load object map file: [{}], {}", sObjectMapFile, err.what() );
		return false;
	}
	...
	if ( !maybeObjects )
	{
		VORTEK_ERROR( "Failed to load object map file: \"game_objects\" table is missing or invalid." );
		return false;
	}
...
bool TilemapLoader::LoadTilemapFromLuaTable( VORTEK_CORE::ECS::Registry& registry, const sol::table& sTilemapTable )
{
	...
	{
		VORTEK_ERROR( "Failed to load tilemap map from table. Table is invalid." );
		return false;
	}
...
bool TilemapLoader::LoadGameObjectsFromLuaTable( VORTEK_CORE::ECS::Registry& registry, const sol::table& sObjectTable )
{
	...
	{
		VORTEK_ERROR( "Failed to load object map from table. Table is invalid." );
		return false;
	}
	...
	if ( !maybeObjects )
	{
		VORTEK_ERROR( "Failed to load objects map file: \"game_objects\" table is missing or invalid." );
		return false;
	}
