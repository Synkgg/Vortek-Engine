...
void PlayerStart::CreatePlayer( VORTEK_CORE::ECS::Registry& registry )
{
	...
	else
	{
		...
		auto& sprite = characterEnt.AddComponent<SpriteComponent>(
			SpriteComponent{ .sTextureName = "ZZ_default_player", .width = 16, .height = 16, .layer = 6 } );

		sprite.bIsoMetric = m_SceneRef.GetMapType() == EMapType::IsoGrid;

		auto pTexture = ASSET_MANAGER().GetTexture( "ZZ_default_player" );
		VORTEK_ASSERT( pTexture && "The default player texture must exist." );
...
void PlayerStart::SetPosition( const glm::vec2& position )
{
	...
	VORTEK_ASSERT( transform && "Visual entity was not setup correctly" );
...
void PlayerStart::LoadVisualEntity()
{
	if ( m_bVisualEntityCreated )
	{
		VORTEK_ERROR( "Failed to load visual entity. Already created." );
		return;
	}
	...
	auto& sprite = m_pVisualEntity->AddComponent<SpriteComponent>(
		SpriteComponent{ .sTextureName = "ZZ_PlayerStart", .width = 64, .height = 64, .layer = 999999 } );

	auto pTexture = MAIN_REGISTRY().GetAssetManager().GetTexture( sprite.sTextureName );
	VORTEK_ASSERT( pTexture && "ZZ_PlayerStart texture must be loaded into the asset manager!" );
