...
void add_handler( EventDispatcher& dispatcher, const sol::table& handler /*, LuaHandler<TEvent>& handler */ )
{
	if ( !handler.valid() )
	{
		VORTEK_ERROR( "Failed to add new event handler. Handler was invalid." );
		return;
	}
...
void remove_handler( EventDispatcher& dispatcher, const sol::table& handler )
{
	if ( !handler.valid() )
	{
		VORTEK_ERROR( "Failed to add remove handler. Handler was invalid." );
		return;
	}
...
void emit_event( EventDispatcher& dispatcher, const sol::table& event )
{
	if ( !event.valid() )
	{
		VORTEK_ERROR( "Failed to emit event. Event was invalid." );
		return;
	}
...
void enqueue_event( EventDispatcher& dispatcher, const sol::table& event )
{
	if ( !event.valid() )
	{
		VORTEK_ERROR( "Failed to enqueue event. Event was invalid." );
		return;
	}
