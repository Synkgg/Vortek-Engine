...
void VORTEK_CORE::State::CreateLuaStateBind(sol::state& lua)
{
	lua.new_usertype<State>(
		...
		[](State& state, sol::protected_function on_enter) {
			if (!on_enter.valid())
			{
				VORTEK_ERROR("Failed to set the on_enter function");
				return;
			}
		...
		[](State& state, sol::protected_function on_exit) {
			if (!on_exit.valid())
			{
				VORTEK_ERROR("Failed to set the on_exit function");
				return;
			}
		...
		[](State& state, sol::protected_function on_update) {
			if (!on_update.valid())
			{
				VORTEK_ERROR("Failed to set the on_update function");
				return;
			}
		...
		[](State& state, sol::protected_function on_render) {
			if (!on_render.valid())
			{
				VORTEK_ERROR("Failed to set the on_render function");
				return;
			}
		...
		[](State& state, sol::protected_function handle_inputs) {
			...
			{
				VORTEK_ERROR("Failed to set the handle_inputs function");
				return;
			}
		...
		[](State& state, const sol::table& table) {
			if (!table.valid())
			{
				VORTEK_ERROR("Failed to set the variables function");
				return;
			}
