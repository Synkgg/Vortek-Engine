...
bool TextureLoader::LoadTexture( const std::string& filepath, GLuint& id, int& width, int& height, bool blended )
{
	...
	if ( !image )
	{
		VORTEK_ERROR( "SOIL failed to load image [{0}] -- {1}", filepath, SOIL_last_result() );
		return false;
	}
										   ...
										   int& height, bool blended )
{
	...
	if ( id == 0 )
	{
		VORTEK_ERROR( "Failed to load texture from memory." );
		return false;
	}
...
bool TextureLoader::LoadIconTexture( const std::string& filepath, GLuint& id, int& width, int& height )
{
	...
	if ( !file )
	{
		VORTEK_ERROR( "Failed to open ICO file: {}", filepath );
		return false;
	}
	...
	else
	{
		...
		{
			VORTEK_ERROR( "Only 32-bit BMP icons or PNG icons are supported." );
			return false;
		}
...
{
	VORTEK_ASSERT( type == Texture::TextureType::FRAMEBUFFER && "Must be framebuffer type" );

	if ( type != Texture::TextureType::FRAMEBUFFER )
	{
		VORTEK_ERROR( "Failed to create texture for frame buffer. Incorrect type passed in!" );
		return nullptr;
	}
