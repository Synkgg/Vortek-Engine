...
bool Framebuffer::Initialize()
{
	...
	{
		VORTEK_ASSERT( false && "Failed to create an OpenGL Framebuffer!" );

		std::string error = std::to_string( glGetError() );
		VORTEK_ERROR( "Failed to create an OpenGL framebuffer: {}", error );
		return false;
	}
	...
	, m_bUseRbo{ bUseRbo }
{
	...
	{
		VORTEK_ASSERT( false && "Failed to create Framebuffer!" );
		VORTEK_ERROR( "Framebuffer creation failed" );
	}
}
...
void Framebuffer::CheckResize()
{
	...
	VORTEK_ASSERT( m_pTexture && "New Texture cannot be nullptr!" );
