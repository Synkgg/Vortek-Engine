...
std::shared_ptr<Font> FontLoader::Create( const std::string& fontPath, float fontSize, int width, int height )
{
	...
	if ( fontStream.fail() )
	{
		VORTEK_ERROR( "Failed to load font [{}] - Unable to read buffer!", fontPath );
		return nullptr;
	}
	...
	{
		VORTEK_ASSERT( false && "Failed to initialize Font Info." );
		VORTEK_ERROR( "Failed to initialize Font Info." );
		return nullptr;
	}
