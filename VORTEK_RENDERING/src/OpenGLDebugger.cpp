...
void OpenGLDebugger::init()
{
	...
	VORTEK_LOG( "Setting up OpenGL debug callback (requires OpenGL version > 4.3.)" );
...
#endif // __linux
			{
				VORTEK_WARN( "There is no valid OpenGL context bound." );
				s_init = false;
				return;
			}
	}
	...
	{ // Checking OpenGL version
		...
		if ( version < 43 )
		{
			VORTEK_WARN( "OpenGL version < 4.3...abort" );
			return;
		}
		else
		{
			VORTEK_LOG( "All good, OpenGL debug callback set properly" );
		}
	}
