...
namespace VORTEK_SOUNDS {
	...
	void SoundFxPlayer::Play(SoundFX& soundFx, int loops, int channel)
	{
		...
		{
			VORTEK_ERROR("Failed to play SoundFX[{}] on channel [{}] -- SoundFX* is null", soundFx.GetName(), channel);
			return;
		}
		...
		if (Mix_PlayChannel(channel, soundFx.GetSoundFxPtr(), loops) == -1)
		{
			...
			VORTEK_ERROR("Failed to play SoundFX[{}] on channel[{}] -- ERROR: {}", soundFx.GetName(), channel, error);
		}
	}
	...
	void SoundFxPlayer::Stop(int channel)
	{
		...
		{
			std::string error{ Mix_GetError() };
			VORTEK_ERROR("Failed to halt soundfx for channel [{}]", channel == -1 ? "all channels" : std::to_string(channel));
		}
	}
