...
{
	VORTEK_ASSERT( pSceneObject && "The SceneObject cannot be nullptr." );

	if ( !pSceneObject )
	{
		VORTEK_ERROR( "Failed to undo remove tile layer. SceneObject was not set correctly." );
		return;
	}
	...
	if ( !pRegistry )
	{
		VORTEK_ERROR( "Failed to undo remove tile layer. Scene Registry was invalid." );
		return;
	}
...
void AddTileLayerCmd::undo()
{
	...
	if ( !pSceneObject )
	{
		VORTEK_ERROR( "Failed to undo add tile layer. SceneObject was not set correctly." );
		return;
	}
...
void AddTileLayerCmd::redo()
{
	...
	if ( !pSceneObject )
	{
		VORTEK_ERROR( "Failed to redo add tile layer. SceneObject was not set correctly." );
		return;
	}
...
void MoveTileLayerCmd::undo()
{
	...
	auto* pRegistry = pSceneObject->GetRegistryPtr();
	VORTEK_ASSERT( pRegistry && "Registry must be valid." );
...
void ChangeTileLayerNameCmd::undo()
{
	...
	if ( !pSceneObject )
	{
		VORTEK_ERROR( "Failed to undo change tile layer name. SceneObject was not set correctly." );
		return;
	}
...
void ChangeTileLayerNameCmd::redo()
{
	...
	if ( !pSceneObject )
	{
		VORTEK_ERROR( "Failed to redo change tile layer name. SceneObject was not set correctly." );
		return;
	}
