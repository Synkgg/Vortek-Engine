	...
	: m_RuntimeRegistry{}
{
	...
	{
		VORTEK_WARN( "Tilemap file [{}] does not exist.", m_sTilemapPath );
	}
...
void SceneObject::CopyPlayerStartToRuntimeRegistry( VORTEK_CORE::ECS::Registry& runtimeRegistry )
{
	if ( !m_bUsePlayerStart )
	{
		VORTEK_ERROR( "Failed to copy player to runtime. Not enabled." );
		return;
	}
...
{
	VORTEK_ASSERT( entity != entt::null && "The entity passed in must be valid." );

	if ( m_mapTagToEntity.contains( sTag ) )
	{
		VORTEK_ERROR( "Failed to add entity with tag [{}] - Already Exists!", sTag );
		return false;
	}
...
bool SceneObject::DeleteGameObjectByTag( const std::string& sTag )
{
	...
	if ( objItr == m_mapTagToEntity.end() )
	{
		VORTEK_ERROR( "Failed to delete game object with tag [{}]. Does not exist or was not mapped correctly.", sTag );
		return false;
	}
...
void SceneObject::OnEntityNameChanges( VORTEK_EDITOR::Events::NameChangeEvent& nameChange )
{
	...
	if ( m_mapTagToEntity.contains( nameChange.sNewName ) )
	{
		...
		VORTEK_ERROR( "Failed to change entity name. [{}] already exists.", nameChange.sNewName );
		return;
	}
