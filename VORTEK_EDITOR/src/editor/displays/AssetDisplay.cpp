...
unsigned int AssetDisplay::GetTextureID( const std::string& sAssetName ) const
{
	...
	case Vortek::Utilities::AssetType::SCENE: {
...
bool AssetDisplay::DoRenameAsset( const std::string& sOldName, const std::string& sNewName ) const
{
	...
	if ( m_eSelectedType == Vortek::Utilities::AssetType::SCENE )
	{
	...
	if ( ImGui::Selectable( ICON_FA_FILE_ALT " Open File Location" ) )
	{
		if ( m_eSelectedType == Vortek::Utilities::AssetType::SCENE )
		{
			...
			if ( !sScenePath.empty() )
			{
				...
				if ( !fp.OpenFileLocation( Vortek::Filesystem::NormalizePath( sScenePath ) ) )
				{
					VORTEK_ERROR( "Failed to open file location [{}]", sScenePath );
				}
			}
		}
		else
		{
			...
			{
				Vortek::Filesystem::FileProcessor fp{};
				if ( !fp.OpenFileLocation( Vortek::Filesystem::NormalizePath( sAssetPath ) ) )
				{
					VORTEK_ERROR( "Failed to open file location [{}]", sAssetPath );
				}
			}
		}
	}
...
{
	if ( m_eSelectedType == Vortek::Utilities::AssetType::MUSIC )
	{
	...
	else if ( m_eSelectedType == Vortek::Utilities::AssetType::SOUNDFX )
	{
...
void AssetDisplay::DrawSelectedAssets()
{
	...
	if ( ImGui::BeginTable( "Assets", numCols, tableFlags ) )
	{
		...
		for (auto assetItr = assetNames.begin(); assetItr != assetNames.end(); ++assetItr, ++id)
		{
			...
			if ( m_eSelectedType == Vortek::Utilities::AssetType::PREFAB )
			{
