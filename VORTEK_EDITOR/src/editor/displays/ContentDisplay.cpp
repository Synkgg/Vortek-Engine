	...
	if ( ImGui::BeginTable( "Content", numCols, IMGUI_NORMAL_TABLE_FLAGS ) )
	{
		...
		if ( popID == 0 )
		{
			if ( ImGui::BeginPopupContextWindow() )
			{
				...
				ImGui::ItemToolTip( "Generates an empty lua class with a Vortek state implementation." );
...
void ContentDisplay::HandleFileEvent( const VORTEK_EDITOR::Events::FileEvent& fileEvent )
{
	...
	case EFileAction::Delete: {
		...
		{
			VORTEK_ERROR( "Failed to delete [{}] - This is a reserved default project folder or file.", path.string() );
			return;
		}
	...
	if ( ImGui::BeginPopupModal( "Create Lua Class" ) )
	{
		...
		if ( bNameEntered && !className.empty() )
		{
			...
			else
			{
				...
				lw.AddWords( className + std::format( " = Class(\"{}\")", className ) )
