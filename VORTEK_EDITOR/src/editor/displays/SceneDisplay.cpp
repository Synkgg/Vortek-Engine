...
void SceneDisplay::LoadScene()
{
	...
	auto mainScript =
		runtimeRegistry.AddToContext<MainScriptPtr>( std::make_shared<VORTEK_CORE::Scripting::MainScriptFunctions>() );
...
{
	if ( !ImGui::Begin( ICON_FA_IMAGE "Scene", nullptr, ImGuiWindowFlags_NoCollapse ) )
	{
		ImGui::End();
		return;
	}
