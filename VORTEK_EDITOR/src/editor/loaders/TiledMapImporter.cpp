...
bool TiledMapImporter::ImportTilemapFromTiled( EditorSceneManager* pSceneManager, const std::string sTiledMapFile )
{
	...
	VORTEK_ERROR( "Failed to import tiled map. File [{}] is not supported.", sTiledMapFile );
	return false;
}
...
bool TiledMapImporter::ImportFromTMXFile( EditorSceneManager* pSceneManager, const std::string sTiledMapFile )
{
	...
	if ( pXMLDoc->LoadFile( sTiledMapFile.c_str() ) != XML_SUCCESS )
	{
		...
		VORTEK_ERROR( "Failed to load tiled map file [{}] - {}", sTiledMapFile, sError );
		return false;
	}
		...
		for ( int row = 0; row < ROWS; row++ )
		{
			for ( int col = 1; col < COLS; col++ )
			{
				...
				if ( auto* pTileset = GetTileset( tilesets, id ) )
				{
					...
					if ( !pTexture )
					{
						VORTEK_ERROR( "Failed to import tiled map. Texture [{}] must be loaded in the asset manager "
									 "prior to import.",
									 pTileset->sName );
						return false;
					}
										  ...
										  const std::string sTiledMapFile )
{
	...
	{
		VORTEK_ERROR( "Failed to import Tiled map. {}", err.what() );
		return false;
	}

	if ( !map.valid() )
	{
		VORTEK_ERROR( "Failed to import Tiled map. Map table invalid. {}" );
		return false;
	}
