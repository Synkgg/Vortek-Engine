...
void Packager::FinishPackaging()
{
	...
	try
	{
		if ( fs::exists( m_pPackageData->sTempDataPath ) )
		{
			...
			if ( numFiles > 0 )
			{
				VORTEK_LOG( "Successfully deleted temp packaging files. [{}]", numFiles );
			}
		}
	}
	catch ( const fs::filesystem_error& err )
	{
		VORTEK_ERROR( "Failed to delete temp data at path [{}] - {}", m_pPackageData->sTempDataPath, err.what() );
	}
...
void Packager::RunPackager()
{
	if ( !m_pPackageData )
	{
		VORTEK_ERROR( "Failed to run packager." );
		return;
	}
	...
	try
	{
		...
		{
			VORTEK_ERROR( "Failed to package game. [{}] directory already exists.", m_pPackageData->sFinalDestination );
		...
		{
			VORTEK_ERROR( "Failed to create directory [{}] - {}", m_pPackageData->sFinalDestination, ec.message() );
		...
		if ( !fs::exists( fs::path{ m_pPackageData->sTempDataPath } ) )
		{
			...
			{
				VORTEK_ERROR( "Failed to create temporary data path at [{}]", m_pPackageData->sTempDataPath );
		...
		VORTEK_ASSERT( optScriptListPath && "Script List Path Must be set." );
		if ( !optScriptListPath )
		{
			VORTEK_ERROR( "Failed to add scripts. Script list path was not set in the project info." );
		...
		{
			VORTEK_ERROR( "Failed to add scripts." );
			UpdateProgress( 25.f, "Packaging failed. Please see logs." );
			m_bHasError = true;
			return;
		}
		...
		VORTEK_ASSERT( optProjectFilePath && "Project File path must be set in project info." );
		if ( !optProjectFilePath )
		{
			VORTEK_ERROR( "Failed to open project file. Project file path was not set in the project info." );
		...
		if ( !procFile.is_open() )
		{
			VORTEK_ERROR( "Failed to open project file [{}]", optProjectFilePath->string() );
			UpdateProgress( 50.f, "Packaging failed. Please see logs." );
			m_bHasError = true;
			return;
		}
		...
		{
			VORTEK_ERROR( "Failed to load project: File [{}] - Expecting \"assets\" member in project file.",
						 optProjectFilePath->string() );
			UpdateProgress( 50.f, "Packaging failed. Please see logs." );
			m_bHasError = true;
			return;
		...
		else
		{
			...
			{
				VORTEK_ERROR( "Failed to create asset defs file." );
			...
			{
				VORTEK_ERROR( "Failed to find the assetDefs.lua file at path [{}].", sAssetDefsFile );
		...
		{
			VORTEK_ERROR( "Failed to load project: File [{}] - Expecting \"scenes\" member in project file.",
						 optProjectFilePath->string() );
		...
		if ( sceneFiles.empty() )
		{
			VORTEK_ERROR( "Failed to create scene files or scene files are invalid." );
		...
		for ( const auto& sSceneFile : sceneFiles )
		{
			...
			{
				VORTEK_ERROR( "Failed to add scene file [{}] to scripts to be compiled.", sSceneFile );
				UpdateProgress( 75.f, "Packaging failed. Please see logs." );
				m_bHasError = true;
				return;
			}
		...
		VORTEK_ASSERT( optMainLuaScript && "Main Lua script not set in the project info." );
		if ( !optMainLuaScript )
		{
			VORTEK_ERROR( "Failed to add main.lua script. Path not set in the project info." );
		...
		{
			VORTEK_ERROR( "Failed to find the main.lua file at path [{}].", optMainLuaScript->string() );
			UpdateProgress( 80.f, "Packaging failed. Please see logs." );
			m_bHasError = true;
			return;
		}
		...
		{
			VORTEK_ERROR( "Failed to add main.lua script." );
			m_bHasError = true;
			return;
		}
		...
		{
			VORTEK_ERROR( "Failed to create config file while packaging." );
	...
	{
		VORTEK_ERROR( "Failed to package game. Error: {}", ex.what() );
		UpdateProgress( 0.f, "Packaging failed. Please see logs." );
		m_bHasError = true;
		return;
	}
...
std::string Packager::CreateConfigFile( const std::string& sTempFilepath )
{
	...
	{
		VORTEK_ERROR( "Failed to create settings config file. [{}] path does not exist.", sTempFilepath );
		return {};
	}
	...
	{
		VORTEK_ERROR( "Failed to create settings config file [{}]. Error: {}", sSettingsConfigPath, ex.what() );
		return {};
	}
...
std::string Packager::CreateAssetDefsFile( const std::string& sTempFilepath, const rapidjson::Value& assets )
{
	...
	if ( !optProjectFilePath )
	{
		VORTEK_ERROR( "Failed to create asset defs file. Project file path was not set in project info." );
		return {};
	}
	...
	{
		VORTEK_ERROR( "Failed to create asset defs file [{}]. Error: {}", sAssetsDefPath, ex.what() );
		return {};
	}
	...
	if ( assets.HasMember( "textures" ) )
	{
		...
		if ( !textures.IsArray() )
		{
			VORTEK_ERROR( "Failed to create asset defs file. Project File [{}]. Expecting \"textures\" as an array.",
						 optProjectFilePath->string() );
			return {};
		}
	...
	if ( assets.HasMember( "soundfx" ) )
	{
		...
		if ( !soundfx.IsArray() )
		{
			VORTEK_ERROR( "Failed to create asset defs file. Project File [{}]. Expecting \"soundfx\" as an array.",
						 optProjectFilePath->string() );
			return {};
		}
	...
	if ( assets.HasMember( "music" ) )
	{
		...
		if ( !music.IsArray() )
		{
			VORTEK_ERROR( "Failed to create asset defs file. Project File [{}]. Expecting \"music\" as an array.",
						 optProjectFilePath->string() );
			return {};
		}
	...
	if ( assets.HasMember( "fonts" ) )
	{
		...
		if ( !fonts.IsArray() )
		{
			VORTEK_ERROR( "Failed to create asset defs file. Project File [{}]. Expecting \"fonts\" as an array.",
						 optProjectFilePath->string() );
			return {};
		}
	...
	{
		VORTEK_ERROR( "Failed to append assets file and add function." );
		return {};
	}
...
std::vector<std::string> Packager::CreateSceneFiles( const std::string& sTempFilepath, const rapidjson::Value& scenes )
{
	...
	if ( !scenes.IsArray() )
	{
		VORTEK_ERROR( "Failed to create scene lua files. \"scenes\" was not a valid json array." );
		return sceneFiles;
	}

	auto optContentPath = m_pPackageData->pProjectInfo->TryGetFolderPath( EProjectFolderType::Content );
	VORTEK_ASSERT( optContentPath && "Content path was not set in project info." );
	if ( !optContentPath )
	{
		VORTEK_ERROR( "Failed to create scene lua files. Content path not set in project info." );
		return sceneFiles;
	}
...
void Packager::CopyFilesToDestination()
{
	...
	VORTEK_LOG( "Listing files in: {}\n.", currentPath.string() );
	VORTEK_LOG( "Copy Destination: {}", m_pPackageData->sFinalDestination );
	...
	try
	{
		...
		{
			VORTEK_ERROR( "Failed to copy files from temp data path." );
		}
		else
		{
				...
				if ( entry.path().extension() == ".luac" )
				{
					...
					if ( fs::is_regular_file( path ) )
					{
						...
						if ( path.filename().string().find( "config" ) != std::string::npos )
						{
							...
							fs::copy( path, dest, fs::copy_options::overwrite_existing );
							VORTEK_LOG( "Copied file [{}] to [{}]", path.filename().string(), configPath.string() );
						...
						else if ( path.filename().string().find( "master.luac" ) != std::string::npos )
						{
							...
							fs::copy( path, dest, fs::copy_options::overwrite_existing );
							VORTEK_LOG( "Copied file [{}] to [{}]", path.filename().string(), scriptPath.string() );
						}
					}
				}
			}
		...
		for ( const auto& entry : fs::directory_iterator( destination ) )
		{
			if ( entry.path().filename().string() == "VORTEK_ENGINE.exe" )
			{
				if ( auto optFileIconPath = m_pPackageData->pProjectInfo->GetFileIconPath() )
				{
					...
					{
						VORTEK_ERROR( "Failed to replace icon file with [{}]", optFileIconPath->string() );
					}
				}
	...
	{
		VORTEK_ERROR( "Failed to copy files to destination: [{}] - {}", m_pPackageData->sFinalDestination, err.what() );
		return;
	}
...
void Packager::CopyAssetsToDestination()
{
	...
	VORTEK_ASSERT( optAssetPath && "Assets folder was not set in project info." );

	if ( !optAssetPath )
	{
		VORTEK_ERROR( "Faield to copy the game assets. Assets folder was not set in project info." );
		return;
	}

	if ( !fs::exists( *optAssetPath ) )
	{
		VORTEK_ERROR( "Failed to copy the game assets to the destination. " );
		return;
	}
	fs::path destination{ m_pPackageData->sFinalDestination };
	destination /= "assets";
	...
	for ( fs::recursive_directory_iterator itr( *optAssetPath ), end; itr != end; ++itr )
	{
		...
		{
			VORTEK_ERROR( "Failed to copy asset file: {}", err.what() );
		}
	}
}
