...
bool ScriptCompiler::AddScripts( const std::string& sScriptList )
{
	...
	{
		VORTEK_ERROR( "Failed to add scripts. Script list file [{}] is invalid.", sScriptList );
		return false;
	}
	...
	try
	{
		...
		if ( !optScriptList )
		{
			VORTEK_ERROR( "Failed to add scripts. [{}] is missing \"ScriptList\" table or is invalid.", sScriptList );
			return false;
		}
		...
		for ( const auto& [ _, script ] : *optScriptList )
		{
			...
			{
				VORTEK_ERROR( "Failed to add script: [{}] - File does not exist.", sFilepath );
				return false;
			}
	...
	{
		VORTEK_ERROR( "Failed to add scripts. Error: {}", error.what() );
		return false;
	}
...
bool ScriptCompiler::AddScript( const std::string& sScript )
{
	...
	{
		VORTEK_ERROR( "Failed to add script [{}] - File does not exist.", sScript );
		return false;
	}
...
void ScriptCompiler::Compile()
{
	...
	VORTEK_LOG( "Successfully compiled lua files in [{}]", m_sOutFile );
}
