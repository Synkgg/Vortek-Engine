...
void AssetPackager::PackageAssets( const rapidjson::Value& assets )
{
	try
	{
		...
		{
			VORTEK_ERROR( "Failed to compile the assets." );
			return;
		}

		if ( !CreateAssetsZip() )
		{
			VORTEK_ERROR( "Failed to archive files successfully." );
			return;
		}
	}
	catch ( const std::exception& ex )
	{
		VORTEK_ERROR( "Failed to package assets: {}", ex.what() );
	}
}
...
bool AssetPackager::CompileLuaAssetFiles()
{
	...
	for ( const auto& entry : fs::directory_iterator( fs::path{ m_Params.sAssetsPath } ) )
	{
		...
		if ( fs::is_regular_file( entry.path() ) && entry.path().extension() == ".veasset" )
		{
			if ( !scriptCompiler.AddScript( entry.path().string() ) )
			{
				VORTEK_ERROR( "Failed to add script: [{}] to asset packager.", entry.path().string() );
				return false;
			}
...
bool AssetPackager::CreateAssetsZip()
{
	...
	if ( !fs::exists( assetsDestination ) )
	{
		...
		{
			VORTEK_ERROR( "Failed to create directory [{}]", assetsDestination.string() );
			return false;
		}
	}

	assetsDestination /= "VortekAssets.zip";
	...
	zip.setErrorHandlerCallback(
		[]( const std::string& message, const std::string& strerror, int zip_error_code, int system_error_code ) {
			VORTEK_ERROR( "Failed to archive assets: Message: [{}] - Error: [{}]", message, strerror );
		} );

	if ( !zip.open( libzippp::ZipArchive::Write ) )
	{
		VORTEK_ERROR( "Failed to open zip: {}", zip.getPath() );
		return false;
	}

	if ( !zip.addEntry( std::string{ "VortekAssets/" } ) )
	{
		VORTEK_ERROR( "Failed to add entry to archive." );
		zip.close();
		return false;
	}
	...
	{
		if ( !zip.addFile( fmt::format( "{}{}{}", "VortekAssets", PATH_SEPARATOR, "textures.luac" ), texturePath ) )
		{
			VORTEK_ERROR( "Failed to add textures.luac to zip." );
			zip.close();
			return false;
		}
	}
	...
	{
		if ( !zip.addFile( fmt::format( "{}{}{}", "VortekAssets", PATH_SEPARATOR, "music.luac" ), musicPath ) )
		{
			VORTEK_ERROR( "Failed to add music.luac to zip." );
			zip.close();
			return false;
		}
	}
	...
	{
		if ( !zip.addFile( fmt::format( "{}{}{}", "VortekAssets", PATH_SEPARATOR, "soundfx.luac" ), soundFxPath ) )
		{
			VORTEK_ERROR( "Failed to add soundfx.luac to zip." );
			zip.close();
			return false;
		}
	}
	...
	{
		if ( !zip.addFile( fmt::format( "{}{}{}", "VortekAssets", PATH_SEPARATOR, "fonts.luac" ), fontsPath ) )
		{
			VORTEK_ERROR( "Failed to add fonts.luac to zip." );
			zip.close();
			return false;
		}
	}
																		...
																		VORTEK_UTIL::AssetType eAssetType )
{
	...
	if ( assets.HasMember( sAssetTypeName.c_str() ) )
	{
		...
		pLuaSerializer->StartNewTable( "Assets" );
