...
bool IconReplacer::Impl::ReplaceIcon_Win()
{
	...
	if ( !file )
	{
		VORTEK_ERROR( "Failed to replace icon: Icon file could not be opened." );
		if ( file.fail() )
			VORTEK_ERROR( "Failed to replace icon: Stream failbit set (invalid format or operation)." );
		if ( file.bad() )
			VORTEK_ERROR(
				"Failed to replace icon: Stream badbit set (irrecoverable error, e.g.hardware I/O failure.)." );
		if ( !file.good() )
			VORTEK_ERROR( "Failed to replace icon: General stream is not good." );
	...
	if ( iconDir.count != 1 )
	{
		VORTEK_ERROR( "Failed to replace icon: Image count is not 1 for [{}].\n"
	...
	if ( !hUpdate )
	{
		VORTEK_ERROR( "Failed to replace icon: Unable to open update handle." );
		return false;
	}
	...
	if ( !UpdateResourceA( hUpdate, RT_ICON, MAKEINTRESOURCEA( 1 ), 0, image.data(), entry.bytesInRes ) )
	{
		...
		VORTEK_ERROR( "Failed to update RT_ICON." );
		return false;
	}
