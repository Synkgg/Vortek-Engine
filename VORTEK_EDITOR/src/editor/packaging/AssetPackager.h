...
{
  public:
	AssetPackager( const AssetPackagerParams& params, std::shared_ptr<VORTEK_UTIL::ThreadPool> pThreadPool );
	~AssetPackager();

	void PackageAssets( const rapidjson::Value& assets );

  private:
	void ConvertAssetToLuaTable( VORTEK_FILESYSTEM::LuaSerializer& luaSerializer, const AssetConversionData& conversionData );
  ...
  private:
	...
	std::shared_ptr<VORTEK_UTIL::ThreadPool> m_pThreadPool;
};
