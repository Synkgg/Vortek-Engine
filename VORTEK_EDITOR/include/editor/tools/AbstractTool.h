...
class AbstractTool
{
  ...
  private:
	...
	void CheckOutOfBounds( const Vortek::Core::Canvas& canvas );
  ...
  protected:
	...
	Vortek::Core::ECS::Registry* m_pRegistry{ nullptr };
	Vortek::Rendering::Camera2D* m_pCamera{ nullptr };
  ...
  public:
	 ...
	 */
	virtual void Update( Vortek::Core::Canvas& canvas );

	/*
	 * @brief Sets the current registry to the registry of the current scene and the camera.
	 */
	bool SetupTool( SceneObject* pSceneObject, Vortek::Rendering::Camera2D* pCamera );
