...
class TileTool : public AbstractTool
{
  ...
  protected:
	std::shared_ptr<Vortek::Rendering::SpriteBatchRenderer> m_pBatchRenderer;
	std::shared_ptr<struct Tile> m_pMouseTile;

  protected:
	uint32_t CheckForTile( const glm::vec2& position );

	Vortek::Core::ECS::Entity CreateEntity();
	Vortek::Core::ECS::Entity CreateEntity( uint32_t id );
