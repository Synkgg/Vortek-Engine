...
RuntimeApp::RuntimeApp()
	...
	, m_Event{}
	, m_bRunning{ true }
	, m_pGameConfig{ std::make_unique<Vortek::Core::GameConfig>() }
{
}
...
{
	VORTEK_INIT_LOGS( true, false );
	VORTEK_INIT_CRASH_LOGS();

	auto basePath = std::filesystem::path( SDL_GetBasePath() );
	...
	VORTEK_CRASH_LOGGER().SetLuaState( pLuaState->lua_state() );

	// Setup Crash Tests
	Vortek::Core::Scripting::CrashLoggerTests::CreateLuaBind( *pLuaState );

	auto& coreGlobals = CORE_GLOBALS();

	// Create the Window
	m_pWindow = std::make_unique<Vortek::Windowing::Window>( m_pGameConfig->sGameName.c_str(),
	...
	auto mainScript = mainRegistry.AddToContext<MainScriptPtr>( std::make_shared<Vortek::Core::Scripting::MainScriptFunctions>() );
	if ( !LoadShaders() )
	{
	...
	auto pSceneManagerData = mainRegistry.AddToContext<std::shared_ptr<Vortek::Core::SceneManagerData>>(
		std::make_shared<Vortek::Core::SceneManagerData>() );
...
bool RuntimeApp::LoadConfig( sol::state& lua )
{
	...
	{
		VORTEK_ERROR( "Failed to load configuration. [{}] config file does not exist.", sConfigFile );
		return false;
	}
...
bool RuntimeApp::LoadRegistryContext()
{
	...
	if ( coreGlobals.IsPhysicsEnabled() )
	{
		auto pPhysicsWorld = mainRegistry.AddToContext<Vortek::Physics::PhysicsWorld>(
			std::make_shared<b2World>( b2Vec2{ 0.f, coreGlobals.GetGravity() } ) );

		auto pContactListener = mainRegistry.AddToContext<std::shared_ptr<Vortek::Physics::ContactListener>>(
...
void RuntimeApp::LoadBindings()
{
	...
	Vortek::Core::SceneManager::CreateLuaBind( *pLuaState, *pRegistry );
}
...
bool RuntimeApp::LoadPhysics()
{
	...
	auto& mainRegistry = MAIN_REGISTRY();
	auto* pRegistry = mainRegistry.GetRegistry();
	auto& pPhysicsWorld = mainRegistry.GetContext<Vortek::Physics::PhysicsWorld>();
...
bool RuntimeApp::LoadZip()
{
	...
	const std::string zipAssetsPath{ fmt::format( "{}{}{}", "assets", PATH_SEPARATOR, "VortekAssets.zip" ) };
	...
	for ( const auto& entry : entries )
	{
		...
		{
			VORTEK_ERROR( "Failed to read in assets! {}", error.what() );
		}

		sol::optional<sol::table> VEAsset = lua[ "Assets" ];
		if ( VEAsset )
		{
			for ( const auto& [ index, assetTable ] : *VEAsset )
			{
				sol::table asset = assetTable.as<sol::table>();
				auto pVEAsset = std::make_unique<Asset>();
				pVEAsset->sName = asset[ "assetName" ].get_or( std::string{ "" } );
				pVEAsset->eType = StrToAssetType( asset[ "assetType" ].get_or( std::string{ "" } ) );
				pVEAsset->assetSize = asset[ "dataSize" ].get_or( 0U );
				pVEAsset->assetEnd = asset[ "dataEnd" ].get_or( 0U );

				if ( pVEAsset->eType == Vortek::Utilities::AssetType::FONT )
				{
					pVEAsset->optFontSize = asset[ "fontSize" ].get_or( 32.f );
				}
				else if ( pVEAsset->eType == Vortek::Utilities::AssetType::TEXTURE )
				{
					pVEAsset->optPixelArt = asset[ "bPixelArt" ].get_or( false );
				}
				...
				for ( const auto& [ _, data ] : dataTable )
				{
					...
					pVEAsset->assetData.push_back( value );
				}

				m_mapVEAssets[ pVEAsset->eType ].push_back( std::move( pVEAsset ) );
			}
		}
	}

	for ( const auto& [ eType, assets ] : m_mapVEAssets )
	{
		switch ( eType )
		{
		case AssetType::TEXTURE: {
			for ( const auto& pTexAsset : assets )
			{
				...
				{
					VORTEK_ERROR( "Failed to add texture [{}] from memory.", pTexAsset->sName );
				}
			}
			break;
		}
		case AssetType::MUSIC: {
			for ( const auto& pMusicAsset : assets )
			{
				...
				{
					VORTEK_ERROR( "Failed to add music [{}] from memory.", pMusicAsset->sName );
				}
			}
			break;
		}
		case AssetType::SOUNDFX: {
			for ( const auto& pSoundfxAsset : assets )
			{
				...
				{
					VORTEK_ERROR( "Failed to add music [{}] from memory.", pSoundfxAsset->sName );
				}
			}
			break;
		}
		case AssetType::FONT: {
			for ( const auto& pFontAsset : assets )
			{
				...
				{
					VORTEK_ERROR( "Failed to add font [{}] from memory.", pFontAsset->sName );
				}
			}
			break;
		}
...
void RuntimeApp::Update()
{
	...
	if ( dt < Vortek::Core::TARGET_FRAME_TIME )
	{
		std::this_thread::sleep_for( std::chrono::duration<double>( Vortek::Core::TARGET_FRAME_TIME - dt ) );
	}
	...
	if ( coreGlobals.IsPhysicsEnabled() && !coreGlobals.IsPhysicsPaused() )
	{
		auto& pPhysicsWorld = mainRegistry.GetContext<Vortek::Physics::PhysicsWorld>();
		pPhysicsWorld->Step(
			Vortek::Core::TARGET_FRAME_TIME_F, coreGlobals.GetVelocityIterations(), coreGlobals.GetPositionIterations() );
		pPhysicsWorld->ClearForces();

		auto& dispatch = mainRegistry.GetContext<std::shared_ptr<Vortek::Core::Events::EventDispatcher>>();

		// If there are no listeners for contact events, don't emit event
		if ( dispatch->HasHandlers<Vortek::Core::Events::ContactEvent>() )
		{
			if ( auto& pContactListener = mainRegistry.GetContext<std::shared_ptr<Vortek::Physics::ContactListener>>() )
			{
				...
				if ( pUserDataA && pUserDataB )
				{
					try
					{
						auto ObjectA = std::any_cast<Vortek::Physics::ObjectData>( pUserDataA->userData );
						auto ObjectB = std::any_cast<Vortek::Physics::ObjectData>( pUserDataB->userData );
...
#ifdef _DEBUG
	if ( INPUT_MANAGER().GetKeyboard().IsKeyJustPressed( VORTEK_KEY_F2 ) )
	{
