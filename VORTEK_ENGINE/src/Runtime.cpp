...
void RuntimeApp::Initialize()
{
	...
	// Setup Crash Tests
	VORTEK_CORE::Scripting::CrashLoggerTests::CreateLuaBind( *pLuaState );

	auto& coreGlobals = CORE_GLOBALS();

	// Create the Window
	m_pWindow = std::make_unique<VORTEK_WINDOWING::Window>( m_pGameConfig->sGameName.c_str(),
	...
	auto mainScript =
		mainRegistry.AddToContext<MainScriptPtr>( std::make_shared<VORTEK_CORE::Scripting::MainScriptFunctions>() );
...
bool RuntimeApp::LoadRegistryContext()
{
	...
	if ( coreGlobals.IsPhysicsEnabled() )
	{
		auto pPhysicsWorld = mainRegistry.AddToContext<VORTEK_PHYSICS::PhysicsWorld>(
			std::make_shared<b2World>( b2Vec2{ 0.f, coreGlobals.GetGravity() } ) );

		auto pContactListener = mainRegistry.AddToContext<std::shared_ptr<VORTEK_PHYSICS::ContactListener>>(
...
bool RuntimeApp::LoadPhysics()
{
	...
	auto& mainRegistry = MAIN_REGISTRY();
	auto* pRegistry = mainRegistry.GetRegistry();
	auto& pPhysicsWorld = mainRegistry.GetContext<VORTEK_PHYSICS::PhysicsWorld>();
...
bool RuntimeApp::LoadZip()
{
	...
	for ( const auto& entry : entries )
	{
		...
		sol::optional<sol::table> sveAsset = lua[ "Assets" ];
		if ( sveAsset )
		{
			for ( const auto& [ index, assetTable ] : *sveAsset )
			{
				sol::table asset = assetTable.as<sol::table>();
				auto pVEAsset = std::make_unique<VEAsset>();
				...
				if ( pVEAsset->eType == VORTEK_UTIL::AssetType::FONT )
				{
					pVEAsset->optFontSize = asset[ "fontSize" ].get_or( 32.f );
				}
				else if ( pVEAsset->eType == VORTEK_UTIL::AssetType::TEXTURE )
				{
	...
	for ( const auto& [ eType, assets ] : m_mapVEAssets )
	{
		...
		case AssetType::FONT: {
			for ( const auto& pFontAsset : assets )
			{
				...
				{
					VORTEK_ERROR( "Failed to add music [{}] from memory.", pFontAsset->sName );
				}
			}
			break;
		}
...
void RuntimeApp::Update()
{
	...
	if ( dt < VORTEK_CORE::TARGET_FRAME_TIME )
	{
		std::this_thread::sleep_for( std::chrono::duration<double>( VORTEK_CORE::TARGET_FRAME_TIME - dt ) );
	}
	...
	if ( coreGlobals.IsPhysicsEnabled() && !coreGlobals.IsPhysicsPaused() )
	{
		auto& pPhysicsWorld = mainRegistry.GetContext<VORTEK_PHYSICS::PhysicsWorld>();
		pPhysicsWorld->Step(
			VORTEK_CORE::TARGET_FRAME_TIME_F, coreGlobals.GetVelocityIterations(), coreGlobals.GetPositionIterations() );
		pPhysicsWorld->ClearForces();

		auto& dispatch = mainRegistry.GetContext<std::shared_ptr<VORTEK_CORE::Events::EventDispatcher>>();

		// If there are no listeners for contact events, don't emit event
		if ( dispatch->HasHandlers<VORTEK_CORE::Events::ContactEvent>() )
		{
			if ( auto& pContactListener = mainRegistry.GetContext<std::shared_ptr<VORTEK_PHYSICS::ContactListener>>() )
			{
				...
				if ( pUserDataA && pUserDataB )
				{
					try
					{
						auto ObjectA = std::any_cast<VORTEK_PHYSICS::ObjectData>( pUserDataA->userData );
						auto ObjectB = std::any_cast<VORTEK_PHYSICS::ObjectData>( pUserDataB->userData );
