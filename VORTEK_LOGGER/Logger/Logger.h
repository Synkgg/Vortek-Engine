...
#define VORTEK_LOG(x, ...) VORTEK_LOGGER::Logger::GetInstance().Log(x, __VA_ARGS__)
#define VORTEK_WARN(x, ...) VORTEK_LOGGER::Logger::GetInstance().Warn(x, __VA_ARGS__)
#define VORTEK_ERROR(x, ...) VORTEK_LOGGER::Logger::GetInstance().Error(std::source_location::current(), x, __VA_ARGS__)
