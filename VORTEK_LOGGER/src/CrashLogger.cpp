...
void CrashLogger::Initialize()
{
	if ( m_bInitialized )
	{
		VORTEK_ERROR( "Failed to initialize CrashLogger - Has already been initialized." );
		return;
	}
	...
	VORTEK_LOG( "Vortek Crash Logger has been initialized." );
}
...
void CrashLogger::SetProjectPath( const std::string& sProjectPath )
{
	...
	{
		VORTEK_ERROR( "Failed to set project path. {}", ec.message() );
	...
	if ( !fs::exists( fs::path{ sLogPath } ) )
	{
		...
		{
			VORTEK_ERROR( "Failed to create crash logs directory.", ec.message() );
			m_sProjectPath.clear();
			return;
		}
	}
...
void CrashLogger::LaunchCrashReporter( const std::string& sFilename )
{
	...
	CreateProcessA(
		"VORTEK_CRASH_REPORTER.exe",															  // Path to crash reporter
		const_cast<char*>( fmt::format( "VORTEK_CRASH_REPORTER.exe {}", sFilename ).c_str() ), // Command line args
		nullptr,
		nullptr,
		FALSE,
		0,
	...
	if ( pid == 0 )
	{
		...
		std::string executable = "./VORTEK_CRASH_REPORTER";
		std::string arg0 = "VORTEK_CRASH_REPORTER";
...
void CrashLogger::CrashHandler( int signal )
{
...

	{
		...
		VORTEK_ASSERT( pLogFile && "Log File was not successfully created and opened." );

		// There is no file to log to, finish log write to console and exit
		if ( !pLogFile )
		{
			VORTEK_ERROR( "Failed to write to crash log file" );
