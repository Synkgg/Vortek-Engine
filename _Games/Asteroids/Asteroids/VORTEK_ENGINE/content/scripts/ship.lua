...
function Ship:AddLives(lives)
	self.numLives = clamp(self.numLives + lives, 0, 3)
...
function Ship:UpgradeWeapon()
	...
	self.currentWeapon = clamp(self.currentWeapon + 1, 0, 3)
...
function Ship:TakeDamage(amount)
	...
	if self.bShieldEnabled then
		self.shieldHealth = clamp(self.shieldHealth - amount, 0, 100)
		...
		self.shipHealth = clamp(self.shipHealth - amount, 0, 100)
