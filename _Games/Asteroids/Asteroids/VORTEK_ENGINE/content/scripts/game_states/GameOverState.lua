...
function GameOverState:Init(params)
	...
	asteroidTransform.position = vec2(
		(WindowWidth() * 0.5) - (asteroidSprite.width * 0.5),
		(WindowHeight() * 0.5) - (asteroidSprite.height * 0.5)
	)
...
function GameOverState:UpdateScreenAnim()
	...
	if asteroidTransform.scale.x < 15.0 then 
		...
		asteroidTransform.position = vec2(
			(WindowWidth() * 0.5) - (asteroidSprite.width * 0.5 * asteroidTransform.scale.x),
			(WindowHeight() * 0.5) - (asteroidSprite.height * 0.5 * asteroidTransform.scale.y)
		)
