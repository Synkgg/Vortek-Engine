...
function Hud:UpdateHealth(event)
	local ship = gShipHandler:GetShipByID(event.data.event.shipID)
	if not ship then 
		warn("Failed to update health. Ship [%d] is invalid", event.data.event.shipID)
		return 
	end
	...
	healthFillTransform.scale.x = clamp(self.lifeFillScaleX * healthPct, 0.0, self.lifeFillScaleX)
...
function Hud:UpdateShield(event)
	local ship = gShipHandler:GetShipByID(event.data.event.shipID)
	if not ship then 
		warn("Failed to update Shield. Ship [%d] is invalid", event.data.event.shipID)
		return 
	end
	...
	local shieldFillTransform = self.shieldBarFill:getComponent(Transform)
	shieldFillTransform.scale.x = clamp(self.lifeFillScaleX * shieldPct, 0.0, self.lifeFillScaleX)
...
function Hud:HandleShipEvent(event)
	switch(event.data.type, 
		...
		function() warn("Not a valid case!") end
	)
end
