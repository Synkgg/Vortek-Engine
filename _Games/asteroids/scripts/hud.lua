...
function Hud:Create()
	...
	-- Create our lives Entities
	for i = 0, gData:MaxLives() - 1 do
		local livedID = LoadEntity(HudDefs["lives"])
		local livesEntity = Entity(livedID)
		local transform = livesEntity:get_component(Transform)
		transform.position = vec2(40 * i, 0.0)
		table.insert(this.m_Lives, livedID)
	end
...
function Hud:SetNumberPositions()
	...
	transform2.position.x = transform1.position.x + (40 * transform2.scale.x)
	local transform3 = Entity(self.m_HundredID):get_component(Transform)
	transform3.position.x = transform2.position.x + (40 * transform3.scale.x)
	local transform4 = Entity(self.m_TensID):get_component(Transform)
	transform4.position.x = transform3.position.x + (40 * transform4.scale.x)
	local transform5 = Entity(self.m_OnesID):get_component(Transform)
	transform5.position.x = transform4.position.x + (40 * transform5.scale.x)
end
...
function Hud:SetScore(current_score)
	-- Get Digit Values
	local ten_thousand = GetDigit(current_score, 5)
