...
function Ship:Create(def)
	...
	{
		m_EntityID = def.id,
		m_ForwardSpeed = def.fwd_speed or 0.13,
		m_AngularSpeed = def.ang_speed or 0.06,
		m_DriftSpeed = def.drift_speed or 0.01,
		m_DriftAngle = vec2(
			math.cos(0),
		    math.sin(0)
		),
...
function Ship:UpdateShip()
	...
	local forward = vec2(
	    math.sin(math.rad(transform.rotation)), -- x Value
	   -math.cos(math.rad(transform.rotation)) -- y Value
	)
	...
	elseif Keyboard.pressed(KEY_D) then -- Clockwise
		transform.rotation = transform.rotation + self.m_AngularSpeed * speedUp
	end
